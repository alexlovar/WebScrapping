...
namespace CarScraping
{
    class WebScrap
    {
        ...
        public void DoScrap()
        {
            ...
            {
                string log = string.Format("Country : {0}, page : {1} ", _countryInfo.Name,i+1);
        ...
        private void StoreImage(CarInfo carInfo, bool isPlate)
        {
            ...
            {
                string log = string.Format("download {0} -> {1}", srcPath, validDstPath);
                ...
                } catch (Exception e)
                {
                    ...
                    log = string.Format("error download : {0} -> {1} \n", srcPath, validDstPath);
